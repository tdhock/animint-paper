...
Another non-animated example is Figure~\ref{fig:ChIP-seq}, which was
...
user to show how the predictions change as a function of the model
...
Animint implements several linked brushes, one of the hallmarks of
interactive visual analysis \citep{iva}. However, the main drawback to
...
expression (e.g. year $>1990$). Importantly, this is a drawback of the
current implementation, not the Animint DSL. 

Another limitation of the current implementation is that axes and
legends are computed at compile-time, and are not updated after
clicks. Thus it is difficult to use interactivity to visualize
different data subsets which have very different ranges of values. So
for example, it would be awkward to use \texttt{showSelected} to show
one panel of a scatter plot matrix of several variables. A workaround
is shown in Figure~\ref{fig:ChIPseq}, which omits the x axis on the
bottom plot, since in fact the x values are all normalized to [0,1]. A
future implementation of Animint would benefit with scales that update
after each click.

Currently, the user is unable to change variable mappings, since these
...
hiding data subsets, there are some limitations to what can be
displayed with multiple selection variables. For instance, in the
...
including the addition of a web server to perform computations on the
