...
every 3 seconds (ms = 3000 milliseconds). The \texttt{duration} option
...
values.  \texttt{Animint} conveniently implements widgets to toggle the
selected values.
...
\texttt{Animint} also provides methods for altering the selection via indirect
...
Figure~1. \texttt{Animint} provides a selection menu which shows the full list
...
this information to the JSON plot meta-data file. It also uses ggplot2 to convert data
...
\citep{2013-immens}. 
When common columns appear across the separated TSV files, an extra common TSV is created to save them and the separated TSV files only keep varied columns to avoid redundancy. In such a case, the render firstly downloads the common TSV only once and recovers the selected subset of data based on key columns before drawing. This TSV storing strategy can optimize the disk usage of \texttt{animint}, particularly efficient for ggplots of spatial maps as in Figure~\ref{fig:tornado}.
...
JavaScript/D3 code that renders the TSV and JSON plot meta-data files as
