...
\abstract{Animint is a novel high-level domain-specific language (DSL)
  ...
  grammar of graphics: \texttt{clickSelects} means that clicking this
  element changes the selection, and \texttt{showSelected} means show
  only the elements that correspond to the current selection. After
...
related packages (Table~\ref{tab:packages}).

\subsection{D3}

The current implementation of Animint uses D3 to render interactive
animations in a web browser. D3 is more expressive than animint, but
is also more verbose (TODO: COMPARISON FIGURE).
...
\subsection{ggplot2, ggvis}

The current implementation of Animint extends the declarative syntax
of ggplot2. Strictly speaking, ggplot2 is for non-interactive and
non-animated visualizations.

The authors of ggplot2 are currently working on ggvis, a package for
interactive graphics. The main difference between Animint and ggvis is
the central unit of a visualization. In ggvis the main unit is a
single plot, but in Animint it is a list of several plots.
