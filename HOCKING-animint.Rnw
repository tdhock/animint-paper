...
visualizations. To demonstrate its design, we begin with an 
illustrative example.

Consider the World Bank data set, which consists of several economic variables
measured for 205 countries from 1960 to 2012 \citep{WorldBank}. Figure~1 
shows six variables from this data set: year, country, life expectancy, 
fertility rate, region, and population. In Figure~1, the year 1996 and 
the countries United States and Vietnam are selected, but readers 
are encouraged to explore other selections using the interactive version.
%% TODO: Let's update this example and facet the bar chart by region
%% \url{http://bit.ly/1fzfUdN}.
In the interactive version, the selected value of the year variable 
is automatically incremented which forces the visualization to update
and allows one to visualize changes in the relationship between life 
expectancy and fertility rates over time for a select subset of countries. 
This is just one example where an interactive animation can lead to 
insight and exploratory freedom otherwise not possible using static plots.
The purpose of Animint is to make the creation of such visualizations 
...
which allows others to focus on designing and consuming visualizations.
...
of the flexibility of D3, but it reduces the cognitive effort required
to create visualizations that allow users to quickly show/hide 
...
%% Carson: I think here we should touch on the differences between 
%% continuous (brushing) versus discrete (clicking) selection. One
%% benefit of having a brush is that it is easier to select elements
%% when there is a high density in a small area.
