...
D3 easy for web developers. Animint's DSL abandons the DOM standard
...
We implemented the World Bank data visualization using the ggvis \citep{ggvis} graphical library combined with the recommended shiny package \citep{shiny} in 84 lines of R
code (\url{http://bit.ly/1diUYsg}). 
While ggvis and shiny combined can produce interactive animations, the DSL ggvis provides is much more complex and more limited than Animint. Our implementation using this combination of packages was
...
must wait for the server to respond with the plot data. In addition, shiny requires 
a server with R loaded, complicating deployment to the web significantly. 
