...
choose projections in a tour, and many ways to implement interpolate 
between projections. In this case, we used the R package tourr,
...
As seen in the video of Figure~\ref{fig:tour}, with our current animint 
implementation, points must be selected via mouse click. In this case,
it would be convenient to also have brush selection, as we demonstrate in
Figure~\ref{fig:tour} which implements the same touring example via the
...
Also, importantly for ggplot2 users, it also requires knowledge of the shiny 
web application framework and a new approach to the grammar of graphics. 
...
result, we believe animint provided a more elegant user interface for 
this application.
...
and not a restriction of the framework itself, 
so we discuss possible solutions in Section~\ref{sec:limitations}.
Thus, when touring a large high-dimensional space, where many projections are
needed, ggvis+shiny may be desirable since the projections are 
...
viewing such an application hosted on a remote machine can result in 
staggered animation since GET requests must be performed, processed,
...
geom-specific effects based on the geom's 
\texttt{clickSelects}/\texttt{showSelected} variables.
