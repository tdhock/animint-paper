...
are encouraged to explore other selections using a web browser to view
...
visualize changes in the relationship between life expectancy and
fertility rates for a subset of countries. This is just one example in
...
data point to add it to the set of selected values).
...
Importantly, this is a drawback of the
