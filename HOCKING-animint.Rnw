...
\begin{table*}
  ...
  \caption{Characteristics of nine interactive visualizations built with 
    ...
    (LOC, 80 characters max per line), the total number of data points
    (rows), the median number of data points shown at once (onscreen),
    the number of data columns visualized (variables), 
    the number of clickSelects/showSelected variables (interactive),
    if the plot is animated, the amount of time it takes to compile
    the visualization (seconds), and the corresponding Figure 
    number in this paper (Fig).
  }
\label{tab:examples}
\end{table*}
...
\begin{figure}
  ...
  \caption{Schematic explanation of compilation and rendering the
    ...
    compiled files are combined with data-independent files which
