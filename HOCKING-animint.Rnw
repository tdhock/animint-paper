...
legends are computed at compile-time, and are not updated when the selection changes.
Thus it is difficult to use interactivity to visualize
different data subsets which have very different ranges of values.
...
future implementation of Animint would benefit from scales that update
after each click.
...
changing the Animint DSL. Examples include zooming, 
increase/decrease animation speed, and plot
