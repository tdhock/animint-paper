...
Interactive, animated data visualizations are a useful tool for
...
Finally, Dimensional Charting (dc.js) is another JavaScript library
which can produce interactive visualization consisting of several
linked plots \citep{dc}.
...
and the Graphics Production Language (GPL) in IBM SPSS, but do not
support interactive animations.
...
observed in the United States between 1950 and 2012. Clicking a state
on the map updates the time series bars to show the tornado counts
from that state. Clicking a bar on the time series updates the
selected year.
...
the selected time and region. Clicking any of the 6 plots updates all
of them. This set of 6 linked plots is easily defined with Animint, using only the clickSelects and showSelected aesthetics; the design of Animint's DSL allows for this level of flexibility while using minimal lines of code to define the plots and the relationship between them. 
...
relationships in multivariate data (e.g. using pairwise scatter
