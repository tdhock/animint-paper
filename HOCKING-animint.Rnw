...
SVG. Another Animint with a similar amount of total data is based on
...
(\url{http://bit.ly/1gGYFIV}). Different model parameters and data sets
can be explored by encoding these as Animint interaction variables. 

In particular, the authors have found that Animint is useful for exploring pairwise relationships in multivariate data. With the selection brushing, it is easy to explore outliers and other extreme values in the dataset and to see how those relationships translate to other variables. With traditional non-interactive graphics, it would be much more difficult (if not impossible) to explore individual points within a larger multivariate context. 
