...
\begin{abstract}
  ...
  limitations of Animint, a new R package that implements these
...
limitation of Vega is that it does does not directly support
...
We use a visualization based on the the World Bank data set to explain
...
section, we propose the Animint R package, which implements a new
...
designer can use animint to render a set of ggplots, then explain the
graphical user interface for selecting data subsets. Finally, we
discuss some implementation details of the compiler and renderer,
which only the animint package developers need to be concerned about.

\subsection{Animint renders a list of ggplots}
...
animint, we first put them in a named list:
...
every 3 seconds (3000 milliseconds = ms). The \texttt{duration} option
...
useful to emphasize continuity between objects before and after the
...
data visualization produced by the animint system. As explained in the
previous section, the animint system outputs a set of data files along
with a web page. Opening the web page in a web browser will render the
data visualization and allow the user select different subsets of data
to plot.

When viewing an animint data visualization in a web browser,
...
interfaces. Animint data visualizations are inspired by principle 2,
...
\texttt{clickSelects} aesthetics. Also, Animint creates a multiple
...
Animint also provides methods for altering the selection via indirect
manipulation, which can be useful in certain situations. For example,
say that you want to add Thailand to the selected set of countries in
Figure~1. Animint provides a selection menu which shows the full list
of countries, and allows entering text to search for a particular
country (Figure~\ref{fig:widgets}).
...
\texttt{clickSelects} and \mbox{\texttt{showSelected}} mappings for
