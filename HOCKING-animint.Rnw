...
\begin{figure*}[b!]
  ...
  \caption{This interactive animation of World Bank data was designed
    with only 20 lines of Animint code (\url{http://bit.ly/1rjwX4r}).
...
\begin{figure*}[b!]
  ...
  \caption{Visualization with 4 selection variables used to navigate
    ...
    black signals below. It also contains an href tag, which opens a
...
follows principal 2 ``Physical actions on objects vs. complex syntax''
...
(e.g. y position and color). The data are separated into several CSV
...
compilation from R objects to 2.1MB of uncompressed CSV data files
...
non-animated plots. In fact, five of the eleven examples in
...
available with showSelected \citep{iva}... ? E.g. it would not be
feasible to compute and plot statistics for a multiple selection
variable. For example you may want to compute a smoothing line with
only the selected poinst
