...
\item
 Susan VanderPlas is with Iowa Statistics. E-mail: srvanderplas@gmail.com.
}
...
\abstract{Animint is a novel high level domain-specific language (DSL)
  for defining interactive animated visualizations. It is In other data
  visualization systems, interactive animations are difficult to
  accomplish, since they must be defined in terms of low-level
  operations and sometimes 100s of lines of code. In contrast,
  Animint's high-level DSL can be used to produce a wide variety of
  visualizations with only 10s of lines of code. It works by adding 2
  new aesthetics to the grammar of graphics: \texttt{clickSelects}
  means that clicking this element changes the selection, and
  \texttt{showSelected} means show only the elements that correspond
  to the current selection.} % end of abstract
...
%% Uncomment below to include a teaser figure.
  ...
  \caption{\textbf{Top}: Animint was used to create this visualization
    of World Bank data, which consists of 4 geometric elements across
    2 plots linked using the clickSelects and showSelected aesthetics
    (interactive animation at
    \url{http://bit.ly/1fzfUdN}). \textbf{Left}: a multiple time
    ...
    countries. The text indicates the current selection: ``year =
...
TODO: discussion of related JavaScript libraries.
