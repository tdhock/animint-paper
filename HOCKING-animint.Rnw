...
To compare our animint implementation with similar systems, we 
reimplemented Figure~\ref{fig:worldbank} using Tableau as well as
the R packages animation and ggvis. As shown in Table~\ref{tab:packages}),
...
\subsection{Recreating the World Bank example with ggvis}
