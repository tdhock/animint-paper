...
\begin{abstract}
Animint is a novel high-level domain-specific language (DSL)
  ...
  graphics: clickSelects, which allows users to select
  elements of the plot, and \mbox{showSelected}, which displays only
...
\begin{figure*}[b!]
  ...
  \caption{This interactive animation of World Bank data was designed
    ...
    geometric elements span two linked plots: clicking a clickSelects
    element changes the currently selected year or country, and
    updates the corresponding \mbox{showSelected} elements.
    \textbf{Left plot}: a multiple time series from 1960 to 2012 of
    the life expectancy of 205 countries, with bold lines showing the
    selected countries and a vertical grey tallrect showing the
    selected year.  \textbf{Right plot}: a scatterplot of life
    expectancy versus fertility rate of all countries. The two text
    elements show the current selection: year=1975 and
...
\end{verbatim}
  ...
  \caption{Comparison of code used to define points on a scatterplot.
    ...
    clickSelects and \mbox{showSelected} aesthetics, 
...
visualizations \citep{animint}. To explain exactly what kinds of
visualizations we are interested in creating, we begin with an
example.
...
1960 to 2012 \citep{WorldBank}. The visualization in Figure~1 shows
six variables from this data set: year, country, life expectancy,
fertility rate, region, and population. We have chosen to emphasize
the country United States and the year 1996. The benefit of the
interactive visualization is that the reader or user may interact with
the plot to view data about another country or time period.
...
what \citet{cleveland} called ``direct manipulation'' (TODO:ref the
other paper) --- the user directly specifies data of interest by using
the mouse on the corresponding plotted elements. Our definition of an
``animated'' visualization is one that shows new data points over time
as the user watches. The goal of the Animint DSL that we introduce in
this paper is to make it easy for designers to express visualizations
which are both animated and interactive.
...
new aesthetics: clickSelects and \mbox{showSelected}.
...
of the D3's flexibility to gain expressive power. Thus, designers
familiar with ggplot2 syntax are now able to declare complex
interactive animations in only a few dozen lines of code using the
Animint DSL. This enables R programmers to design a large class of
visualizations that were previously intractable to implement.
...
Animint uses \href{http://d3js.org/}{D3}
...
animations. For example, the rCharts R package is a wrapper
around several JavaScript libraries \citep{rcharts}. The major
difference is that rCharts is more expressive but less flexible
than Animint. Thus, rCharts can produce
...
Animint, DC does not use the grammar of graphics, so DC designers must
...
googleVis package \citep{googleVis}. Unlike Animint, googleVis has no
high level DSL, so it can not be used to produce a multi-plot
visualization with a linked multiple time series.
...
graphics philosophy \citep{wilkinson}: ggplot2, ggvis, and Vega.
...
clickSelects and \mbox{showSelected} aesthetics for
...
\href{http://ggvis.rstudio.com/}{ggvis} \citep{ggvis}. Unlike Animint,
ggvis does not directly extend ggplot2, but provides a new
implementation of some of the same ideas about the grammar of
graphics. There are many other differences between ggvis and Animint,
but the main difference is that ggvis is more powerful and verbose
when used with the shiny web server \citep{shiny}, while Animint is
more succinct for expressing interactive plots that show/hide data
subsets (for a concrete example, see the comparison of World Bank data
visualizations in Table~\ref{tab:packages}). At the level of defining
plots in R code, a ggvis object corresponds to a single plot, while an
Animint object is a list of several plots which are linked using the
...
shiny, but Animint uses static files and client-side JavaScript so it
does not require any special web server software. The two packages
also have different interactive features: ggvis uses sliders,
checkboxes, and other HTML form elements, and Animint provides
clickable SVG elements. This usually results in a richer Animint data
visualization using fewer lines of code. For example, in a ggvis of
the WorldBank data, it is natural to use a slider to select the
displayed year. In contrast, in Animint we used a clickable multiple
time series plot for the same purpose, giving the viewer a more
comprehensive view of these multidimensional data (Figure~1).
...
functionality implemented in Animint. There are four important
differences between Animint and this system. First, Animint is based
on a high-level DSL, but Cleveland's system proposes no such
language. Second, Animint supports a brush for every selection
variable, but Cleveland's system only supports a single brush. Third,
the current implementation of Animint only supports selecting single
values, but Cleveland's system supports selecting several
values. Importantly, this is a limitation of the current
implementation, not the proposed Animint DSL. Finally, Animint is
publicly available on GitHub \citep{animint}, but Cleveland's system
...
clickSelects and \mbox{showSelected}. For example, after
...
will be shown in the linked plots. Also, note the clickSelects and
...
Animint implements the clickSelects and showSelected interactive
...
from the same data table; the clickSelects and
\mbox{showSelected} aesthetics will work as long as the different
...
non-selected clickSelects plot elements have 1/2 less
...
subsets than could ever be precomputed. In that case, the web server would only compute the subsets that the user interactively specifies.
...
of them. The clickSelects and showSelected aesthetics make it easy to
...
plots). By using clickSelects, it is easy to explore outliers and
...
interactivity: highlighting the selected clickSelects
element, and showing/hiding \mbox{showSelected} elements. In the
