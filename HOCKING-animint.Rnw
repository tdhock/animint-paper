...
Our definition of ``interactive'' is synonymous to what
\citet{cleveland} calls ``direct manipulation'' --- the user directly
specifies data of interest by clicking the mouse on their
corresponding plotted elements. in the of brushing
\citep{cleveland}. Three modes: transient (data under brush =
selection), lasting (data under brush are added to the selection), or
erasing (data under brush are removed from the selection). Operations:
labeling (show text near selection), enhanced linking (change the
symbol used to plot the selection) deletion (e.g. outliers).
...
%% Table captions on top in journal version
  \caption{Implementation complexity of the World Bank data visualization of 
...
geom. This highlights one of the main motivations for using a
declarative DSL like Animint: none of the designer's R code needs to
be changed to implement improvements like this. Instead, the Animint
