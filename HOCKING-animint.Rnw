...
\begin{figure*}[b!]
  ...
  \caption{This interactive animation of World Bank data was designed
    with only 20 lines of Animint code (TODO:bit.ly URL with new
    version after Carson adds widgets).  \textbf{Top text}: Five
    geometric elements span two linked plots: clicking a
    \texttt{clickSelects} element changes the currently selected year
    or country, and updates the corresponding \texttt{showSelected}
    elements.  \textbf{Left plot}: a multiple time series from 1960 to
    2012 of the life expectancy of 205 countries, with bold lines
    showing the selected countries and a vertical grey tallrect
    showing the selected year.  \textbf{Right plot}: a scatterplot of
    life expectancy versus fertility rate of all countries. The two
    text elements show the current selection: year=1975 and
    country=$\{\textrm{United States}, \textrm{Vietnam}\}$}
  \label{fig:1}
\end{figure*}
...
interested in creating, we begin with an illustrative example.

We are concerned with tabular data sets, with columns for each
variable and rows for each observation or data point. Consider the
World Bank data set, which consist of several economic variables
measured for 205 countries from 1960 to 2012 \citep{WorldBank}. The
visualization in Figure~1 shows six variables from this data set:
year, country, life expectancy, fertility rate, region, and
population. We have chosen to emphasize the year 1996 and the contries
United States and Vietnam. The benefit of the interactive
visualization is that the reader or user may interact with the plot to
view data about another country or time period.
%(\url{http://bit.ly/1fzfUdN}). 
Finally, since one of the variables in this visualization is the year
of measurement, it is natural to use animation to show how the data
changes with respect to the year variable. The purpose of Animint is
to make it easy to design such interactive animations.
...
In this paper we will discuss three important roles that contribute to the
...
Scalable Vector Graphics (SVG) elements in a web browser. A final goal
is that the Animint library developer should be the only role
concerned with low-level plotting details such as drawing axes and
legends, freeing the designer and user to focus on interpreting the
data.
%interested in re-implementing the Animint
%system, we discuss details of its implementation in
%Section~\ref{sec:implementation}.

The goal of the Animint DSL is to make it easy for designers to
express visualizations which are both animated and interactive. For
the purposes of this paper, we define ``interactive'' to mean that
graphical elements such as data points may be clicked to update the
data that is shown in related plots. This is closely related to the
concept of ``direct manipulation,'' which was introduced by
\citet{shneiderman82} and later applied to statistical data
visualization \citep{Hutchins:1985, brushing-scatterplots,
  cleveland}. Our definition of an ``animated'' visualization is one
that can be watched like a video, automatically updating over
time. 
...
\texttt{clickSelects=year} within a ggplot2 aes specification.
...
geometric properties.  The Animint DSL extends ggplot2 by adding
...
of the flexibility of D3, but makes it easier to express one kind of
interaction: showing and hiding subsets of data. 
%% Thus, designers
%% familiar with ggplot2 should find it easy to use the Animing DSL to
%% declare complex interactive animations in only a few dozen lines of R
%% code.
The Animint DSL thus enables design of a large class of complex
interactive data visualizations using just a few lines of code.
...
implementations in JavaScript and R. The main difference is that
Animint's declarative DSL allows interactive animations to be
expressed much more easily than the other libraries.

\subsection{Types of selection}
For example, it is popular to select regions with brushing tools--see
\citep{heer2008generalized}, \citep{scented-widgets} for lots of good
examples of selection modalities. Which of these does the system
support? Do the authors regret losing the ability to multi-select, or
is the grammar theoretically expandable to adding those too? Are these
unnecessary, or uninteresting? Are they HARD to express, or simply
things that the authors didn't get around to doing?
...
abstraction than Animint, so is able to express a wider variety of
...
Animint uses a shorter syntax involving a simple \texttt{aes} mapping
...
\end{verbatim}
  ...
  \caption{Comparison of code used to define points on a scatterplot.
    ...
    Vega code. Also, Animint implements the proposed
...
rCharts does not include a DSL for describing interactions between
multiple plots. Thus rCharts can produce several simple visualizations
with only 1 line of R code, but it can not produce multi-plot interactive
animations. In contrast, Animint's high-level DSL can be used to
specify more complex visualizations using only tens of lines of R
code.
...
Animint, DC does not use the grammar of graphics, so DC designers are
limited to plot types that are pre-defined by the library developers.
...
system is that it is limited to a few pre-defined plot types, only one
...
interactive graphics is called \href{http://ggvis.rstudio.com/}{ggvis}
...
grammar of graphics. There are many other differences between ggvis
and Animint. The main difference is that ggvis can be used to define
single plots, but Animint can be used to define multiple plots which
are linked using the \texttt{clickSelects} and \texttt{showSelected}
aesthetics. Another difference is that sharing a ggvis visualization
requires a web server that runs R and shiny, but Animint uses static
files and client-side JavaScript so it does not require any special
web server software. The two packages also have different interactive
...
since it has less articulatory indirection \citep{Hutchins:1985}, and
...
interactive animation. There main difference between Vega and Animint
is that Vega does not interactions and animation that show and hide
data subsets across multiple linked plots. Finally,
\citet{2014-reactive-vega} propose some declarative interaction
...
The system described by \citet{cleveland, brushing-scatterplots}
involves interaction via a rectangular brush that can select several
data points. Its enhanced linking feature highlights the selected data
points across several plots, which is similar to the selection
functionality implemented in Animint. There main important difference
is that Animint supports a brush for every selection variable, but
Cleveland's system only supports a single brush.

Another system with interactive facilities similar to Animint is
 \citet{tableau}. Like Animint, it is based on a declarative
DSL. The Tableau DSL is called VizQL, which is based on another DSL
called Polaris \citep{polaris}. TODO: CARSON DISCUSS TABLEAU.

Finally, implementations of the grammar of graphics system of
...
In conclusion, other systems that implement the grammar of graphics
are limited to non-interactive plots, and other systems that implement
interactive animations do not exploit the powerful grammar of
graphics. Animint is the first system that implements both.
...
The first step in the design of an Animint is usually to make a sketch
of the desired interactive plot on paper or a whiteboard. First sketch
the axes, legends, a few geometric elements, and note which variables
will be shown in the linked plots. Also, note the \texttt{clickSelects} and
\mbox{showSelected} variables for each geometric element, as shown in
Figures~1, 3, and 4. These notes can be directly translated to
ggplot2 aesthetics in R code, as explained below.

\begin{figure*}[b!]
  \centering
  \includegraphics[width=\textwidth]{screenshot-chip-seq-edited}
  \caption{Visualization containing 5 linked interactive plots of a
    L1-regularized logistic regression model of ChIP-seq data
    (\url{http://bit.ly/1CYob4g}). TODO}
  \label{fig:climate}
\end{figure*}

Animint implements the \texttt{clickSelects} and \texttt{showSelected} interactive
keywords as aesthetics in ggplot2. An aesthetic is a declarative
mapping from a data variable to a characteristic of a geometric plot
...
For single
selection variables such as year, clicking sets the value of the
corresponding selection variable. For multiple selection variables
such as country, clicking adds or removes values from the set of
selected values. 

\citet{instrumental-interaction} discussed direct manipulation in
post-wimp interfaces -- our system follows principal 2 ``Physical
actions on objects vs. complex syntax'' such as dialog boxes.
...
\begin{figure}[b!]
  ...
  \caption{Schematic explanation of compilation and rendering the
    ...
    ggplot geoms to a database of CSV files
...
(e.g. y position and color). The data are separated into separate CSV
...
plots to the web for sharing visualizations with users.
...
declarative grammar to specify interactive animations. Note that is it
possible to implement the World Bank visualization in pure D3, but
would require significantly more code (Figure~\ref{fig:code}).
...
data set. In contrast, the most complex visualization required 87
lines of code, and it shows 15 variables across 6 linked plots
(Figure~\ref{fig:climate}). All of the visualizations that we designed
...
compiled CSV file database (``MB'' column), and found that both
...
geometric elements at once. We observed this slowdown across all
...
SVG. Another Animint with a similar amount of total rows is based on
the evolution data (\url{http://bit.ly/O0VTS4}), but since it shows
less data onscreen (about 2703 elements), it exhibits faster
...
design this set of 6 linked plots in only 87 lines of code. Animint's
...
non-animated plots. In fact, five of the nine examples in
Table~\ref{tab:examples} are non-animated. For example, these plots
...
Also, we have found that Animint is useful for exploring pairwise
relationships in multivariate data (e.g. using several scatter
plots). By using \texttt{clickSelects}, it is easy to explore outliers and
other extreme values in the dataset and to see how those relationships
translate to other variables. With traditional non-interactive
graphics, it would be much more difficult (if not impossible) to
explore individual points within a larger multivariate context.
...
TODO: discuss that AND is the only logical combination variable
available with showSelected \citep{iva}... ?

TODO: discuss that it would be nice to have scales that update for
each selection.

TODO: discuss that it would be nice to interactively change variable
mappings.
...
future, we could implement several other types of interactivity without
...
to the Animint grammar. For example, a \texttt{tooltip} aesthetic
...
could definitely be made even more efficient and responsive. For
example, of the plots in
Table~\ref{tab:examples}, the longest compilation step took 23.7 seconds.
...
animated data visualizations. It is free/open-source software
available on GitHub \citep{animint}.

\section*{Acknowledgements}

The authors wish to thank Animint users MC Du Plessis, Song Liu, and Nikoleta Juretic who have contributed constructive criticism and helped its development.  
