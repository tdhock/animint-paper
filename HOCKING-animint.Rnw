...
code and the ggvis and shiny packages (TODO:BITLY). Because ggvis and
shiny do not have any declarative DSL for interactive animations, this
implementation was significantly more complex than the 20 lines of R
code we used to build Figure~1 with the Animint package. Showing and
hiding data subsets was accomplished by clicking on a slider for year
and a menu for country, not by clicking on the plot elements. Although
creating such visualizations in ggvis+shiny is possible, it requires
