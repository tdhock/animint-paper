...
sets.  For example, consider the World Bank data, which consist of
several economic variables measured for 205 countries from 1960 to
2012. The visualization in Figure~1 shows six variables from this data
set: year, country, life expectancy, fertility rate, region, and
population. As the designers of this visualization and authors of this
paper, we have chosen to emphasize the country United States and the
year 1996. If the reader or user of this interactive data
visualization is interested in another country or time period, he/she
can use interactivity to change the emphasis. Finally, since one of
the variables in this visualization is the year of measurement, it is
natural to animate the visualization to see how the data changes over
time. We introduce Animint, a new domain-specific language (DSL) for
designing such linked, interactive, animated data visualizations. To
explain exactly what kinds of visualizations are possible using
Animint, we begin with several definitions.

For the purposes of this paper, there are 3 important roles that
contribute to the creation of an Animint visualization: the developer,
who implements the Animint library; the designer, who uses the Animint
library to define a visualization; and the user, who views the
visualization in a web browser. We as authors of this paper are the
developers of the Animint library, and we discuss details of its
implementation in Section~\ref{sec:implementation}. The designer
specifies data sets and maps variables to interactive visual elements
using our novel DSL, then uses the Animint library to compile and save
an interactive animation. The user writes no code, but can view and
interact with an Animint visualization by clicking SVG elements in a
web browser.
...
\citep{ggplot2}, which is an R implementation of the grammar of
graphics \citep{wilkinson}. The ggplot2 language was created as a
high-level abstraction for non-interactive visualizations. One of the
key strengths of ggplot2 is that it allows a designer to declare a
visualization in terms of distinct geometric elements, each with a
clear aesthetic mapping from data variables to geometric properties.
The Animint DSL extends ggplot2 by adding two new aesthetics:
\texttt{clickSelects} and \texttt{showSelected}.
...
terminology of the Document Object Model, which makes learning D3 easy
for web developers. Animint's DSL abandons the DOM standard and
sacrifices some of the D3's flexibility to gain expressive
...
code using the Animint DSL. This enables R developers to design a
...
The structure of this paper is as follows: we discuss related work in
Section~2, and the design of the Animint system in Section~3. Then we
discuss a detailed comparison with related libraries in Section~4, and
...
related libraries/packages in JavaScript/R. The main difference is
...
\citep{animation}. TODO: CODE COMPARISON FIGURE.
...
more expressive than animint, but is also more verbose. In addition,
animint uses a grammar of graphics approach to produce higher-level
plots, which is arguably simpler and more elegant than constructing a
plot from simple elements using D3. (TODO: CODE COMPARISON FIGURE)
...
in Section~\ref{sec:compare}). At the level of defining plots in R
...
\subsection{A grammar for interactive animations}
...
Figure~1, year is used to create tallrects and
...
Figure~1, we add a text label that shows the selected
...
in 2 parts:
\begin{description}
\item[compiler] $\approx 1000$ lines of R code that converts a list of
  ggplots and options to a comma-separated values (CSV) file database
  and a JSON plot meta-data file.
\item[renderer] $\approx 1000$ lines of JavaScript/D3 code that
  renders the CSV and JSON plot files as SVG in a web browser.
\end{description}
...
Bank visualization of Figure~1 using several different libraries
...
\subsection{Animation packages}

TODO: discuss animation.

\begin{table}[t!]
%% Table captions on top in journal version
  \caption{Implementation complexity and featureof the World Bank data visualization of 
    Figure~1 using several libraries that can create interactive animations.
    For each library
    we show the number of lines of code (LOC), programming language, 
...
We implemented the World Bank data visualization using 84 lines of R
code and the ggvis and shiny packages
(\url{http://bit.ly/1diUYsg}). 
Because ggvis and shiny do not have any
declarative DSL for interactive animations, this implementation was
significantly more complex than the 20 lines of R code we used to
build Figure~1 with the Animint package. Showing and hiding data
subsets was accomplished by clicking on a slider for year and a menu
for country, not by clicking on the plot elements. Although creating
such visualizations in ggvis+shiny is possible, it requires
significantly more work for limited interactivity.
...
page loading using DevTools in Google Chrome and timing interactive
updates using \texttt{getTime()} in JavaScript. Using ggvis with a
...
since the data sets we examined were sufficiently small
...
implemented with Animint (Table~\ref{tab:examples}). 
