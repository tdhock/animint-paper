...
increase/decrease animation speed, and plot resizing. However, some
kinds of interactivity would require extensions to the Animint
grammar. For example, a \texttt{tooltip} aesthetic could be used to
indicate the text the user views when they hover the mouse over a data
point. Or a \texttt{hoverSelects} aesthetic could be used to change
the selection when hovering over a data point.
