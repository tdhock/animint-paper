...
\begin{abstract}
  ...
  limitations of Animint, an R package that implements these keywords:
...
elements such as data points may be clicked to update the data that is
...
work in Section~2, and the usage of the clickSelects and showSelected
keywords in Section~3. Then we discuss our current implementation in
the animint R package in Section~4.
...
D3 is a JavaScript library that can render interactive, animated data
graphics in a web browser \citep{d3}. One of the main reasons for the
success and popularity of D3 is that it allows visualizations to be
specified using the terminology of the Document Object Model (DOM),
which makes learning D3 easy for web designers. Interactivity in D3 is
specified by writing JavaScript functions that can perform arbitrary
DOM manipulations in response to user events.
...
concepts from the grammar of graphics \citep{vega}. Vega is unable to
express all plots that can be made using pure D3, but provides a JSON
file format capable of defining a wide variety of multi-layer
plots. The main limitation of Vega is that it does does not directly
support interactions and animation. However, the recent work of
...
\section{The clickSelects and showSelected keywords for interactive
  plot design}
\label{sec:design}
...
In this paper we propose to add the clickSelects and showSelected
keywords as new aesthetics in ggplot2's grammar of graphics.  We use a
visualization based on the the World Bank data set to explain the
usage of these new aesthetics. The World Bank data consist of several
economic variables measured for 205 countries from 1960 to 2012
\citep{WorldBank}. Figure~1 shows six variables from this data set:
year, country, life expectancy, fertility rate, region, and
population. In the static PDF version of this figure, the year 1979
and the countries United States and Vietnam are selected, but readers
are encouraged to interactively select other values using a web
browser (TODO:link).
%% TODO: Let's put the code for this example on https://github.com/tdhock/animint/blob/master/inst/examples/WorldBank-paper.R
...
A ggplot2 aesthetic is a declarative mapping from a data variable to
a visual property of a geometric plot element. Some standard ggplot2
aesthetics are
\begin{description}
...
for each year. The multi-layer ggplot2 system allows a new data set
for each layer/geom, and the \texttt{clickSelects} and
\texttt{showSelected} aesthetics will work as long as the different
data sets have common variable names (e.g. the \texttt{year} variable
is present in both \texttt{WorldBank} and \texttt{years}). The
\texttt{label} aesthetic is used to define the text from the year
...
The tallrect is geom which is useful for selecting the variable which
is plotted on the x axis, such as \texttt{year} in this example. The
tallrect plots a rectangle for every row of the \texttt{years} data
table. The rectangle spans the entire y region, and the \texttt{xmin}
and \texttt{xmax} aesthetics define the left and right limits. Since
the designer specified \texttt{clickSelects=year}, users can click on
a tallrect to select a year.
%% Finally,
...
ggplot2 library will render a non-interactive plot will all geometric
elements, including data for all years and countries. In this next
section we explain the Animint R package which implements an
interactive renderer for ggplots with the clickSelects and
...
\section{Current implementation in the Animint R package}
...
Data-Driven Documents (D3) library for JavaScript \citep{d3}. 
...
by clicking on the data objects themselves or by clicking on elements
in the legend which are directly mapped to the data. This contrasts
othersystems which use menus and widgets, and thus suffer from less
articulatory directness \citep{Hutchins:1985}.

While it is typically preferable to click on the plot, there are
situations where menus or widgets are necessary.  When there are many
different elements that could be selected, for example, it can be
convenient to have widgets that contain all the possible selected
values.  Animint conveniently implements widgets to toggle the
selected values.
...
The compiler is implemented in about 1800 lines of R code that
...
the axes scales, legends, labels, backgrounds, and borders. It outputs
this information to
...
The \texttt{animint.js} renderer is implemented in about 2200 lines of
...
\texttt{animation} does not support smooth transitions between
