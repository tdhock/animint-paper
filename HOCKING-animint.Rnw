...
several other high-level DSLs for data visualization: ggplot2, Vega, 
and ggvis.
...
\paragraph{ggvis} The authors of ggplot2 are currently working on ggvis, a package for
...
ggvis is only interactive when run locally or on a server with R and \citep{shiny}, so it 
...
sliders, checkboxes, and other HTML form elements to interact with the plots. 
