...
several other high-level DSLs for data visualization based on the grammar of graphics philosophy \citep{wilkinson}: ggplot2,  ggvis, and Vega.
...
\citep{animint}, but Cleveland's system is
