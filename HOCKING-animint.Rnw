...
implementations in JavaScript and R. In short, Animint supports many 
...
Visual query languages remove the burden of writing database 
queries to begin the data visualization process. 
In fact, the system will automatically and dynamically 
execute (and, in some cases, cache) any queries required to render 
various properties of the user defined visualization. 

One fundamental limitation to a visual query language is the
...
that can transfered are limited.
