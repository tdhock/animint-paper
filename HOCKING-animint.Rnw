...
%% Uncomment below to include a teaser figure.
  ...
  \caption{This interactive animation of World Bank data was designed
    with only 20 lines of Animint code
    (\url{http://bit.ly/1fzfUdN}). \textbf{Top}: Five geometric
    elements span two linked plots: clicking a clickSelects
    element changes the currently selected year or country, and
    updates the corresponding \mbox{showSelected}
    elements. \textbf{Left}: a multiple time series from 1960 to 2012
    of the life expectancy of 205 countries, with a bold line showing
    the selected country and a vertical grey tallrect showing the
    selected year.  \textbf{Right}: a scatterplot of life expectancy
    versus fertility rate of all countries. The two text elements show
    the current selection: ``year = 1996'' and ``United States.''}
}
...
current implementation, not the proposed Animint DSL. Finally, Animint
is publicly available on GitHub \citep{animint}, but Cleveland's
...
code on the server, in response to user inputs, and then sending the
result across the network for plotting in the user's web
browser. However, this power is not always necessary for interactive
...
time for the selected state. Clicking a state on the map updates the
...
example, the compilation step currently saves geom-specific CSV files
and could be improved by saving CSV files for each showSelected subset
of data. This would also allow for faster rendering for large data
sets, since then the plots could be displayed as soon as the first
selection is downloaded. In contrast, the current code downloads all
of the data before displaying any geoms.
