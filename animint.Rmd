---
title: "Extending ggplot2 for linked and animated web graphics on the client"
...
statistical graphics systems such as  `PRIM-9` [@PRIM9], `Dataviewer` 
...
systems allow users to create the graphics from the command-line, which allows
...
[@Unwin:1999vp]. Web technologies can certainly be used to build a similar class 
of system, but in order to capitalize on the key strength of web technologies 
(e.g., accessible, portable, and composable), we should be more mindful about 
what technologies are actually required to accomplish a particular set of 
data analysis tasks.
...
There are now a large number of R packages that interface to purely client-side
graphing libraries and give users the option of embedding these graphics in a 
larger web application, if need be. In fact, this is a large enough use case
...
workflow for "production-ready" exploratory web graphics.
...
graphics with two new aesthetics, `clickSelects` and
`showSelected`. We introduce their usage using the simple example
shown in Figure~1:

* `clickSelects` defines a selection source via a mouse click. For
  example `aes(clickSelects=smoker)` designates a plot element which,
  when clicked, will change the selected value of the `smoker` variable.
* `showSelected` defines a selection target. For example
  `aes(showSelected=smoker)` means to only show a plot element for data
  with the currently selected value of the `smoker` variable.

Below, we show the R code that creates a linked view between a bar chart
and a scatter plot, where the user can click on bars to control the
points shown in the scatterplot
...
As a result, we can interactively explore how the relationship between tip amount
...
```{r tips, echo = FALSE, eval = TRUE, fig.cap = "Linked dynamic querying of tips data set via direct manipulation. Left: the `clickSelects` aesthetic designates a clickable geom bar that can change a selection variable. Right: the `showSelected` aesthetic designates a geom point that responds by showing only the data which corresponds to the current selection."}
...
We will use a visualization of the World Bank data set to explain how
the features we propose generalize to multi-layer plots (Figure
\@ref(fig:worldbank)). The visualization allows interactive
exploration of the relationship between life expectancy and fertility
over time for 205 countries. In this paper, the year 1979 and the
...
version, which is provided in the supplementary materials. In the
interactive version, the selected value of the year variable is
...
rendering options, as we discuss below (see Table~1 for a summary of
...
the bivariate relationship between fertility rate and life expectancy
