...
to graphical systems such as GTK+ [@RGtk2], Qt [@qtbase, @qtpaint], or Java GUI frameworks 
...
the example above, we have used `aes(clickSelects=smoker)` to specify
a plot in which direct manipulation (mouse clicks on the barplot)
dynamically changes the `smoker` selection variable. We have used
`aes(showSelected=smoker)` to designate a scatterplot where we only
want to show data points for the current selection of the `smoker`
variable. In essense, this means performing SQL queries of the form:
...
In this example, `smoker_selection` is either "Yes" or "No", but as we
show in later examples, `smoker_selection` can also be an array of values.
Although the `clickSelects` aesthetic is tied to a mouseclick event,
