...
provides tools for distributing entire research compendium(s) (code, data, 
documentation, auxiliary documents, etc) [@Gentleman:Lang]. 

One of the most widely used R packages is ggplot2, a data
visualization package inspired by the grammar of graphics [@ggplot2-book, @wilkinson]. 
...
to graphical systems such as GTK+, Qt, or Java GUI frameworks 
[@RGtk2, @qtbase, @qtpaint, @rJava]. In general, the resulting system can be 
...
via the Cairo graphics API [@cairo]. The R package SVGAnnotation provides functionality to post-process `svg()` output in order to add interactive 
and dynamic features [@SVGAnnotation]. This is a powerful 
...
`svg()` output. As a result, anyone wishing to extend or alter the 
...
the `d3.transition()` API [@d3]. Smooth transitions help us track
...
ggvis graphics typically requires special web server softwares, even when the
interaction logic could be handled entirely client-side. As we outline in 
Section \@ref(implementation), our system does not require a web server, 
...
dynamic queries via direct manipulation. In the ggplot2 system, there are five
...
`clickSelects` aesthetic for defining the selection source (via mouse 
click) and a `showSelected` aesthetic for defining the target. Here we 
...
In this example, `clickSelects` is either "Yes" or "No", but as we show 
in later examples, `clickSelects` can also be an array of values. 
Although `clickSelects` is tied to a mouseclick event, this same 
...
extension is closely related to trellis 
displays and linked scatterplot brushing 
[@trellis, @brushing-scatterplots]. The major differences are that our conditioning
is layer-specific (not plot-specific), is not tied to a particular geometry, and 
...
altering the `time` and `duration` parameters. 
...
```{r animation, echo = FALSE, eval = TRUE, fig.cap = "Two frames from an animation of one marker moving from (1, 1) towards (10, 10). A video of the animation with smooth transitions can be viewed online at <https://vimeo.com/160505146>"}
...
specify the horizontal range via xmin and xmax. Also, since the
...
the `time` and `duration` options (introduced in 
Section \@ref(animation)) to control the animation parameters. Our animint
...
`first` option for controlling which values are selected by default.
...
`animint2dir()` will write all the files necessary to render 
...
```{r updates, echo = FALSE, eval = TRUE, fig.cap = "Interactive animation of tornadoes in the United States from 1950 to 2012. In this view, the year 1982 and state of Arkansas are selected, so tornado paths from 1982 are displayed on the map and the tornado counts represent Arkansas. The middle and right panel display the same data, but use different scaling: the middle panel reflects the 'global' range (US) while the right panel reflects the 'local' range (Arkansas). Futhermore, when an axis update is triggered, they smoothly transition from one state to next (preserving object constancy in the axis ticks). This helps the viewer better perceive/understand how the range has changed from one state to the next. The interactive version of this chart may be viewed here https://bl.ocks.org/faizan-khan-iit/raw/b3912f21ec2750f96e8d1bd4b66463b2/#year=\{1982\}state=\{AR\} "}
...
a bar on the time series updates the spatial distribution of tornadoes in the selected year. Figure \@ref(fig:climate) 
...
of them. The `clickSelects` and `showSelected` aesthetics make it
...
(<http://bit.ly/O0VTS4>), but since it shows less data onscreen
...
(<http://bit.ly/1gGYFIV>). The user can explore different model
...
For the `prior` data visualization
(<http://bit.ly/1peIT7t>), the animint user is a machine learning
...
(<http://bit.ly/1gVb8To>) helped greatly by rendering an
...
`clickSelects` selection(s) change. Some 
