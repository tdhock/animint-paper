...
of system, but in order to capitalize on the key strengths of web technologies 
...
work entirely with client-side technologies. Unfortunately, client-side technologies are not particularly well-suited for 
...
exploratory graphics that are easily distributed, but also hope to see more 
opinionated approaches to this idea, like **plotly** [@plotly], that focus more on 
...
**animint** which are included with the supplementary materials.
...
approach, since in theory it can work with any `R` graphics, but the
...
The `R` package **ggvis** [@ggvis] is another interactive web graphics interface inspired
by the Grammar of Graphics. It does not directly extend
...
in our supplementary materials, our system does not require server-side `R`, 
...
sections, we further explore options that allow us to accumulate selections,
...
is not very useful in this case since it is not necessarily easy to identify and 
...
visualizations inside of **shiny** web applications -- refer to the supplementary
...
of interaction that could be added without adding to the extension at all 
