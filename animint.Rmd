...
## Direct Manipulation of Database Queries {#extension}
...
```{r tips, echo = FALSE, eval = TRUE, fig.cap = "Linked database querying via direct manipulation using animint. A video demonstration can be viewed online at <https://vimeo.com/160496419>"}
...
We acheive this by reserving the name `time` to specify which variable 
...
series, so in the code beloe we add some tall rectangles to the time series plot.
...
time series on the right hand plot, so we set `clickSelects=country`
...
before render time. As eluded to in Section \@ref(tour),
