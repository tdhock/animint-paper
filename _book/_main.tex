...
\hypersetup{unicode=true,
            pdftitle={Extending ggplot2 for linked and animated web graphics on the client},
...
new aesthetics, \texttt{clickSelects} and \texttt{showSelected}. We
introduce their usage using the simple example shown in
Figure\textasciitilde{}1:

\begin{itemize}
\tightlist
\item
  \texttt{clickSelects} defines a selection source via a mouse click.
  For example \texttt{aes(clickSelects=smoker)} designates a plot
  element which, when clicked, will change the selected value of the
  \texttt{smoker} variable.
\item
  \texttt{showSelected} defines a selection target. For example
  \texttt{aes(showSelected=smoker)} means to only show a plot element
  for data with the currently selected value of the \texttt{smoker}
  variable.
\end{itemize}
...
As a result, we can interactively explore how the relationship between
tip amount and total bill amount depends on whether the customer is
smoker.
...
\caption{\label{fig:tips}Linked dynamic querying of tips data set via direct
manipulation. Left: the \texttt{clickSelects} aesthetic designates a
...
\ref{fig:worldbank}). The visualization allows interactive exploration
of the relationship between life expectancy and fertility over time for
205 countries. In this paper, the year 1979 and the countries United
States and Vietnam are selected, but readers are encouraged to change
the selection by clicking on the interactive version, which is provided
in the supplementary materials. In the interactive version, the selected
value of the year variable is automatically incremented every few
seconds, using animation to visualize yearly changes in the relationship
between life expectancy and fertility rate.
