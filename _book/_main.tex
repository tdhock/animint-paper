...
bar is clicked which triggers our system to perform an SQL query of the
form:
...
very useful in this case since it is not necessaily easy to identify and
select these countries based solely on the graphics. Figure
...
widgets to recompile plot(s) via a callback to R. For this reason,
\textbf{animint} makes it easy to embed visualizations inside of
\textbf{shiny} web applications, and we've provided some examples on
\href{http://members.cbio.ensmp.fr/~thocking/animint-paper-figures/}{our
site} that hosts all the interactive figures for this paper.
...
extention at all (e.g., zooming, panning, and plot re-sizing).
